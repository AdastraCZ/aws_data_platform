create or replace procedure dwh_master.country_overview_map()
language plpgsql    
as $$
begin
--
    CALL dwh_adm.log_event('Start', 'postgres', null,  null, 'dwh_master.country_overview_map', null);
    --
    TRUNCATE TABLE dwh_master.country_overview;

    INSERT INTO dwh_master.country_overview
        ...
        data as (
        select 
            lov.country_iso_3_code as country_id,
            lov.name as country_name,
            lov.capital_city as country_capital,
            lov.region_name as region_name,
            lov.region_iso_2_code as region_code,
            case lov.income_level 
            when 'LIC' then 'Low income' 
            when 'LMC' then 'Lower-middle income' 
            when 'UMC' then 'Upper-middle income' 
            when 'HIC' then 'High income' 
            else 'NA' 
            end as income_level,
    ...
    --
    CALL dwh_adm.log_event('Finish', 'postgres', null, null, 'dwh_master.country_overview_map', null);
--
end;$$