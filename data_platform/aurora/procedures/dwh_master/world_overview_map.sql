create or replace procedure dwh_master.world_overview_map()
language plpgsql    
as $$
begin
--
    CALL dwh_adm.log_event('Start', 'postgres', null, null, 'dwh_master.world_overview_map', null);
    --
    TRUNCATE TABLE dwh_master.world_overview;

    INSERT INTO dwh_master.world_overview
        ...
        -- NA = aggregates region
        where lov.region_iso_2_code != 'NA'
        )
        select 
		...
		group by year, valid_from_date_cet, valid_to_date_cet, create_datetime_cet;
    --
    CALL dwh_adm.log_event('Finish', 'postgres', null, null, 'dwh_master.world_overview_map', null);
--
end;$$